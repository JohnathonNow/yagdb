...
fn test_skip_clause() {
    ...
    // Test SKIP only
    let res = g.execute("MATCH (u:User) RETURN u.name AS name ORDER BY name ASC SKIP 2").unwrap();
    ...
    let res = g.execute("MATCH (u:User) RETURN u.name AS name ORDER BY name ASC SKIP 1 LIMIT 2").unwrap();
    let parsed: Value = serde_json::from_str(&res).unwrap();
    let arr = parsed.as_array().unwrap();
    assert_eq!(arr.len(), 2);
    ...
    let res = g.execute("MATCH (u:User) RETURN u.name AS name ORDER BY name ASC SKIP 10").unwrap();
    let parsed: Value = serde_json::from_str(&res).unwrap();
    assert!(parsed.is_null() || (parsed.is_array() && parsed.as_array().unwrap().is_empty()));

    // Test WITH SKIP
    let res = g.execute("MATCH (u:User) WITH u.name AS name ORDER BY name ASC SKIP 2 RETURN name").unwrap();
    let parsed: Value = serde_json::from_str(&res).unwrap();
    let arr = parsed.as_array().unwrap();
    assert_eq!(arr.len(), 2);
    assert_eq!(arr[0]["name"].as_str().unwrap(), "Charlie");
