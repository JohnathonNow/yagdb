...
fn test_string_functions() {
    ...
    // substring
    let res = g.execute("RETURN substring('hello world', 0.0, 5.0) AS sub").unwrap();
    assert!(res.contains(r#""sub": "hello""#) || res.contains(r#""sub":"hello""#));

    // substring overflow
    let res = g.execute("RETURN substring('hello', 0.0, 100.0) AS sub").unwrap();
    assert!(res.contains(r#""sub": "hello""#) || res.contains(r#""sub":"hello""#));

    // substring out of bounds start
    let res = g.execute("RETURN substring('hello', 10.0, 5.0) AS sub").unwrap();
...
fn test_math_functions() {
    ...
    // round
    let res = g.execute("RETURN round(5.4) AS r1, round(5.5) AS r2").unwrap();
    assert!((res.contains(r#""r1": 5.0"#) || res.contains(r#""r1":5.0"#)) && (res.contains(r#""r2": 6.0"#) || res.contains(r#""r2":6.0"#)));
