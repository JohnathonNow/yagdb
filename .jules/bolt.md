...
## 2026-09-08 - Use Stack Array + Iterator in Recursive Traversal hot loops
**Learning:** In yagdb query execution pipeline (specifically `match_edges_recursive` and `match_var_length_edges`), `bindings` vectors were dynamically allocated on the heap inside deeply recursive and looping graph traversal bodies using `Vec::with_capacity(2)`.
**Action:** Replace `Vec` allocations with stack allocated Arrays (`[Option<(&str, GraphElement)>; 2]`) mapped from `.as_ref()` and pass them to collectors using `.into_iter().flatten()`. This guarantees zero heap allocation on hot recursive edges while bypassing the borrow checker.
