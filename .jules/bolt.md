...
## 2026-09-08 - Eliminate Vec Allocations with Option Arrays
**Learning:** In yagdb's graph traversal logic (e.g., `match_edges_recursive`, `match_var_length_edges`), creating small vectors using `let mut bindings = Vec::with_capacity(2);` inside hot loops causes unnecessary heap allocations for pushing to `ResultSet`.
**Action:** Replace `Vec` creation in deep recursive bindings loops with stack-allocated Option arrays combined with `flatten()`. e.g. `let b1 = ...; let b2 = ...; out.push_row_from(..., IntoIterator::into_iter([b1, b2]).flatten())` to bypass heap allocation entirely.
