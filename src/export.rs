...
impl Graph {
    ...
    pub fn export_csv(&self) -> Result<(String, String, String, String), String> {
        ...
        for i in 0..self.nodes.len_items() {
            // OPTIMIZATION: Use with_item to avoid cloning the full node object and its vectors/maps during export
            let result = self.nodes.with_item(i, |node| -> Result<(), String> {
        ...
        for i in 0..self.edges.len_items() {
            // OPTIMIZATION: Use with_item to avoid cloning the full edge object during export
            let result = self.edges.with_item(i, |edge| -> Result<(), String> {
