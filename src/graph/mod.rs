...
impl Graph {
            ...
            loop {
                ...
                match entry {
                    ...
                    WalEntry::AddNode {
                        ...
                        let node =
                            Node::new(id.clone(), vec![label], vec![], properties.clone(), 0);
                    ...
                    WalEntry::AddEdge {
                        ...
                        graph
                            .nodes
                            .with_mut_item(start, |n| n.edges.push(edge_idx))
                            .unwrap();
                        graph
                            .nodes
                            .with_mut_item(end, |n| n.edges.push(edge_idx))
                            .unwrap();
                    }
                    ...
                    WalEntry::SetNodeLabel { node_id, label_id } => {
                        let mut added = false;
                    ...
                    WalEntry::SetNodeProperty {
                        ...
                        let (old_value, has_label) = graph
                            .nodes
                            .with_mut_item(node_id, |__node| {
                                (
                                    __node.properties.insert(key.clone(), value.clone()),
                                    __node.labels.clone(),
                                )
                            })
                            .unwrap();
                    ...
                    WalEntry::RemoveNodeProperty { node_id, key } => {
                        let (old_value, has_label) = graph
                            .nodes
                            .with_mut_item(node_id, |n| {
                                (n.properties.remove(key.as_str()), n.labels.clone())
                            })
                            .unwrap();
                    ...
                    WalEntry::DeleteEdge { edge_id } => {
                        graph
                            .edges
                            .with_mut_item(edge_id, |e| e.deleted = true)
                            .unwrap();
                    }
                }
                needs_snapshot = true;
    ...
    pub fn element_to_json(&self, element: &GraphElement) -> Value {
        match element {
            GraphElement::Node(node_id) => self
                .nodes
                .with_item(*node_id, |node| {
                ...
                .unwrap(),
            GraphElement::Edge(edge_id) => self
                .edges
                .with_item(*edge_id, |edge| {
                    let mut map = serde_json::Map::new();
                    map.insert(
                        "labels".to_string(),
    ...
    pub fn format_element(&self, element: &GraphElement) -> String {
        match element {
            GraphElement::Node(node_id) => self
                .nodes
                .with_item(*node_id, |n| format!("{:?}", n))
                .unwrap(),
            GraphElement::Edge(edge_id) => self
                .edges
                .with_item(*edge_id, |e| format!("{:?}", e))
                .unwrap(),
            GraphElement::EdgeArray(edge_ids) => {
                let edges_str: Vec<_> = edge_ids
                    .iter()
                    .map(|&id| self.edges.with_item(id, |e| format!("{:?}", e)).unwrap())
                    .collect();
            ...
            std::sync::Arc::new(|args| {
                if args.len() == 3 {
                    if let (
                        ...
                        GraphElement::Number(start),
                        GraphElement::Number(length),
    ...
    pub fn create_index(&self, label: usize, property: String, index_type: IndexType) {
        ...
        self.log_wal(&WalEntry::CreateIndex {
            label,
            property,
            index_type,
        });
        ...
        for step in &plan.steps {
            ...
            match step {
                ...
                ExecutionStep::Match(
                    ...
                    let plan_opt = box_plan_opt.clone().map(|b| *b);
                    let condition_opt = box_condition_opt.clone().map(|b| *b);
                            ...
                            crate::parser::SetItem::Label(var, label) => {
                                ...
                                for i in 0..result_set.rows {
                                        ...
                                        result_set.get(i, var.as_str())
                                    {
                                        ...
                                        if added {
                                            if let Some(label_indices) =
                                                self.indices.write().get_mut(&label_id)
                                            {
                                                for (key, val) in properties {
                                                    if let Some(prop_index) =
                                                        label_indices.get_mut(key.as_str())
                                            ...
                                            self.log_wal(&WalEntry::SetNodeLabel {
                                                node_id,
                                                label_id,
                                            });
                                        }
                    ...
                    for item in items {
                        match item {
                                        ...
                                        result_set.get(i, var.as_str())
                                    {
                                        ...
                                        if updated_nodes.insert((node_id, key.clone())) {
                                            let (old_value, has_label) = self
                                                ...
                                                .with_mut_item(node_id, |n| {
                                                    (
                                                        n.properties.remove(key.as_str()),
                                                        n.labels.clone(),
                                                    )
                                                })
                                                .unwrap();
                            ...
                            crate::parser::RemoveItem::Label(var, label) => {
                                let label_id_opt = self.labels.read().get(label.as_str()).copied();
                                if let Some(label_id) = label_id_opt {
                                    for i in 0..result_set.rows {
                                            ...
                                            result_set.get(i, var.as_str())
                                        {
                                            ...
                                            let (removed, properties) = self
                                                .nodes
                                                .with_mut_item(node_id, |n| {
                                                    let mut removed = false;
                                                    if let Some(pos) =
                                                        n.labels.iter().position(|&l| l == label_id)
                                                    {
                                                        n.labels.remove(pos);
                                                        removed = true;
                                                    }
                ...
                ExecutionStep::With(ref items, ref order_by_opt, s, l)
                | ExecutionStep::Return(ref items, ref order_by_opt, s, l) => {
                    ...
                    let items_vec: Vec<ProjectionItem> =
                        if items.len() == 1 && matches!(items[0], ProjectionItem::Star) {
                            let mut keys: Vec<String> = result_set
                                .columns
                                .keys()
                    ...
                    for item in &items_vec {
                        match item {
                            ...
                            ProjectionItem::Variable(ref _var) => grouping_items.push(item.clone()),
                            ProjectionItem::AliasedVariable(ref _var, _) => {
                                grouping_items.push(item.clone())
                            }
                            ProjectionItem::Property(_, _)
                            | ProjectionItem::AliasedProperty(_, _, _) => {
                                grouping_items.push(item.clone())
                            }
                    ...
                    if has_aggregate {
                        let mut groups: indexmap::IndexMap<Vec<Option<GraphElement>>, Vec<usize>> =
                        ...
                        for i in 0..result_set.rows {
                            ...
                            for item in &grouping_items {
                                let element = match item {
                                    ...
                                    | ProjectionItem::AliasedVariable(var, _) => {
                                        result_set.get(i, var.as_str()).cloned()
                                    }
                                    ProjectionItem::Property(var, prop)
                                    | ProjectionItem::AliasedProperty(var, prop, _) => self
                                        .get_property_as_element(
                                            result_set,
                                            i,
                                            var.as_str(),
                                            prop.as_str(),
                            ...
                            if let Some(group) = groups.get_mut(&key_buf) {
                                ...
                                groups.insert(
                                    std::mem::replace(
                                        &mut key_buf,
                                        Vec::with_capacity(grouping_items.len()),
                                    ),
                                    vec![i],
                                );
                            }
                    ...
                    if let Some(order_items) = order_by_opt {
                        let mut env_with_keys: Vec<(Vec<EvalValue>, usize)> = (0..final_res.rows)
                            .map(|i| {
                                let keys = order_items
                                    .iter()
                                    .map(|item| self.evaluate_expression(&item.expr, &final_res, i))
                                    .collect();
                                (keys, i)
                            })
                            .collect();
                ...
                ExecutionStep::DropIndex {
                    ...
                    if let Some(label_id) = self.labels.read().get(label).cloned() {
                        self.drop_index(label_id, property.clone());
                    }
                }
            }
        }
                    ...
                    for node_id in nodes {
                        if let Some(var) = &pattern.variable {
                            out.push_row_from(
                                in_res,
                                i,
                                [(var.as_str(), GraphElement::Node(node_id))],
                            );
                        } else {
                            out.push_row_from(
                                in_res,
                    ...
                    for p_idx in 0..probe_res.rows {
                        ...
                        if let Some(b_indices) = hash_table.get(&key_buf) {
                            for &b_idx in b_indices {
                                let (l_idx, r_idx) = if build_is_left {
                                    (b_idx, p_idx)
                                } else {
                                    (p_idx, b_idx)
                                };
                                ...
                                for (k, r_col) in &right_res.columns {
                                    if !join_keys.contains(k) {
                                        if let Some(l_col) = left_res.columns.get(k) {
                                            if l_col[l_idx] != GraphElement::Null
                                                && r_col[r_idx] != GraphElement::Null
                                                && l_col[l_idx] != r_col[r_idx]
                                            {
                                                valid = false;
                                                break;
                                            }
    ...
    fn match_var_length_edges(
                ...
                if let Some(target_label_id) = target_label_id {
                    self.nodes
                        .with_item(current_node_id, |node| {
                            self.node_matches(node, target_node_pattern, target_label_id, u64::MAX)
                        })
                        .unwrap()
                } else {
                    false
        ...
        let start_node_edges = self
            .nodes
            .with_item(current_node_id, |n| n.edges.clone())
            .unwrap();
    ...
    fn node_matches(
        ...
        if node.deleted || node.created_by > txid || node.deleted_by.is_some_and(|d| d <= txid) {
            return false;
        }
    ...
    fn find_edges_and_nodes(
        ...
        for &edge_id in &start_node_edges {
            ...
            if let Some(end_node_id) = edge_matches {
                ...
                self.nodes
                    .with_item(end_node_id, |end_node| {
                        if self.node_matches(
                            end_node,
                            target_node_pattern,
                            target_label_id,
    ...
    fn get_property_as_element(
        ...
        if let Some(element) = in_res.get(row_idx, var) {
            let prop_val = match element {
                GraphElement::Node(id) => self
                    .nodes
                    .with_item(*id, |n| n.properties.get(prop).cloned())
                    .unwrap(),
                GraphElement::Edge(id) => self
                    .edges
                    .with_item(*id, |e| e.properties.get(prop).cloned())
                    .unwrap(),
                _ => None,
            };
            match prop_val {
                ...
                Some(crate::property::PropertyValue::DateTime(dt)) => {
                    Some(GraphElement::DateTime(dt))
                }
                None => None,
            }
        } else {
    ...
    fn evaluate_expression_to_element(
        &self,
        expr: &Expression,
        in_res: &ResultSet,
        row_idx: usize,
        ...
        match expr {
            ...
            Expression::Variable(var) => in_res
                .get(row_idx, var)
                .cloned()
                .unwrap_or(GraphElement::Null),
            ...
            Expression::Property(var, prop) => self
                .get_property_as_element(in_res, row_idx, var, prop)
                .unwrap_or(GraphElement::Null),
            Expression::List(elements) => {
                let lst: Vec<GraphElement> = elements
                    .iter()
                    .map(|e| self.evaluate_expression_to_element(e, in_res, row_idx))
                    .collect();
            ...
            Expression::Map(map) => {
                ...
                for (k, v) in map {
                    result_map.insert(
                        k.clone(),
                        self.evaluate_expression_to_element(v, in_res, row_idx),
                    );
                }
...
impl Graph {
    ...
    pub fn replace_from(&self, other: Graph) {
        ...
        *self.functions.write() = other.functions.into_inner();
        self.next_txid.store(
            other.next_txid.load(std::sync::atomic::Ordering::Relaxed),
