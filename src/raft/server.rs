...
async fn handle_batch(
    ...
    for body in queries {
        ...
        let is_write = match q {
            Ok((_, query)) => query.clauses.iter().any(|c| {
                matches!(
                    c,
                    crate::parser::Clause::Create(_)
                        | crate::parser::Clause::Set(_)
                        | crate::parser::Clause::Delete(_)
                        | crate::parser::Clause::Remove(_)
                        | crate::parser::Clause::Merge(_)
                        | crate::parser::Clause::CreateIndex { .. }
                        | crate::parser::Clause::DropIndex { .. }
                )
            }),
            Err(_) => false,
        };
                    ...
                    match e {
                        openraft::error::RaftError::APIError(
                            ...
                            if let Some(leader_node_id) = fwd.leader_id {
                                let url =
                                    format!("http://127.0.0.1:{}/query", 3000 + leader_node_id);
                                let client = reqwest::Client::new();
                                let resp =
                                    client.post(&url).body(body).send().await.map_err(|e| {
                                        (
